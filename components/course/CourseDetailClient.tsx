          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-start">
            <div className="space-y-6">
              ...
              <div className="mt-4">
                ...
                {showShareOptions && (
                  <div id={`share-options-${courseContent.id}`} className="mt-4 space-y-3">
                    {!shareUrl && (
                      <div className="text-sm text-purple-500 text-center">
                        {isAr ? 'جاري تحضير روابط المشاركة...' : 'Preparing share options...'}
                      </div>
                    )}
                    {shareUrl && (
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        <FacebookShareButton url={shareUrl}>
                          <div className="flex items-center gap-3 border border-purple-100 rounded-lg p-2 hover:bg-purple-50 transition-colors">
                            ...
                            <span className="text-sm font-medium text-purple-800">{isAr ? 'فيسبوك' : 'Facebook'}</span>
                          </div>
                        </FacebookShareButton>
                        <TwitterShareButton url={shareUrl} title={displayTitle}>
                          <div className="flex items-center gap-3 border border-purple-100 rounded-lg p-2 hover:bg-purple-50 transition-colors">
                            <TwitterIcon size={36} round />
                            <span className="text-sm font-medium text-purple-800">{isAr ? 'تويتر' : 'Twitter'}</span>
                          </div>
                        </TwitterShareButton>
                        <LinkedinShareButton url={shareUrl}>
                          <div className="flex items-center gap-3 border border-purple-100 rounded-lg p-2 hover:bg-purple-50 transition-colors">
                            <LinkedinIcon size={36} round />
                            <span className="text-sm font-medium text-purple-800">{isAr ? 'لينكدإن' : 'LinkedIn'}</span>
                          </div>
                        </LinkedinShareButton>
                        <WhatsappShareButton url={shareUrl}>
                          <div className="flex items-center gap-3 border border-purple-100 rounded-lg p-2 hover:bg-purple-50 transition-colors">
                            <WhatsappIcon size={36} round />
                            <span className="text-sm font-medium text-purple-800">{isAr ? 'واتساب' : 'WhatsApp'}</span>
                          </div>
                        </WhatsappShareButton>
                        <TelegramShareButton url={shareUrl} title={displayTitle}>
                          <div className="flex items-center gap-3 border border-purple-100 rounded-lg p-2 hover:bg-purple-50 transition-colors">
                            <TelegramIcon size={36} round />
                            <span className="text-sm font-medium text-purple-800">{isAr ? 'تيليجرام' : 'Telegram'}</span>
                          </div>
                        </TelegramShareButton>
                      </div>
                    )}
            ...
            {activeTab === 'curriculum' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="text-sm text-purple-600">{courseContent.curriculum.length} {isAr ? 'دروس' : 'lessons'} • {durationLabel}</div>
                </div>
                <div className="bg-white border border-purple-100 rounded-xl overflow-hidden shadow-sm">
                  {courseContent.curriculum.map((lesson, index) => (
                    <div key={index} className={`px-6 py-4 flex items-center justify-between transition-colors ${index !== courseContent.curriculum.length - 1 ? 'border-b border-purple-50' : ''} hover:bg-purple-25`}>
                      <div className={isAr ? 'flex items-center space-x-reverse space-x-4' : 'flex items-center space-x-4'}>
                        ...
                        <div className="flex-1">
                          <div className={isAr ? 'flex items-center space-x-reverse space-x-3' : 'flex items-center space-x-3'}>
                            <h5 className="font-medium text-purple-900">{isAr ? arabicContent.curriculum[index] ?? lesson.title : lesson.title}</h5>
                            {lesson.isPreview && (
...
function buildArabicOverrides(course: CourseDetailRecord, english: CourseViewModel): ArabicContent {
  const lessonNames =
    course.lessons && course.lessons.length > 0
...
function formatTotalDuration(minutes: number, locale: 'en' | 'ar'): string {
  ...
  if (hours === 0) {
    return locale === 'ar' ? `${remainingMinutes} دقيقة` : `${remainingMinutes} minutes`
  }
